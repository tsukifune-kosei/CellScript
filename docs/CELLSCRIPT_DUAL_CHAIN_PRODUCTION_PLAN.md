...
| Molecule schema/ABI | 100/100 | Public VM/CellScript ABI is Molecule-based, metadata schema 30 emits an authoritative schema manifest, bundled examples produce schema manifest report artifacts, and unsupported generic dynamic shapes remain fail-closed instead of silently compiling. |
