...
| Script identity API | Distinguish address decoding, full Script construction/hash comparison and signature verification; wrong-domain values fail typing or checked conversion. | Partially implemented: typed transaction views expose complete hashes as `ScriptHash`; `ckb::script_hash(Hash)` is an explicit domain conversion; authoring `exact_hash` rejects `Address` and raw `Hash`; fixed `ExactScriptHandle` values bind an admitted artifact/deployment receipt to a selected full Lock/Type Script hash or verifier CellDep data hash; and the deployment-line foundation hash-links Type-hash exact versions under derived six-dimensional compatibility and active/yanked policy. Address parsing, deployment-line admission/runtime checks, generic interface-compatible handles, source-level complete Script construction, and signature verification remain pending. |
