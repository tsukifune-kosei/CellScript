...
The 0.30 branch now implements step 1 and the deterministic offline conflict
core of step 2 as `cellscript-protocol-bundle-v1`. Concrete Molecule
transaction materialization and byte-identical multi-group dry-runs remain
open, so source choreography is still premature.
...
| [#9 Artifact-only multi-Script ProtocolBundle](https://github.com/CellScript-Labs/CellScript/issues/9) | **[ALIGNED] [OVERLAP] [IMPLEMENTED FORMAT] [PREREQUISITE]** | The 0.30 branch implements the artifact-only schema, standalone artifact admission, canonical hash, and deterministic offline conflict matrix. A future `.celltx` file must remain a frontend over `cellscript-protocol-bundle-v1` or its accepted successor. Builder-derived claims, concrete transaction serialization, and byte-identical multi-group execution are still prerequisites for source choreography. |
