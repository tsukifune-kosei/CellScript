...
| [#11 Interface-bound runtime Script handles](https://github.com/CellScript-Labs/CellScript/issues/11) | **[ALIGNED] [IMPLEMENTED EXACT ARTIFACT IDENTITY] [PREREQUISITE]** | `cellscript-exact-script-handle-receipt-v1` and `CSHDLv1-fixed-202` now bind exact independently checked ProtocolBundle artifacts and deployments while remaining ordinary non-Cell values. Generic source `ScriptHandle<I>` / `VerifierHandle<I>`, checked runtime construction, upgrade-line identity, compatible-open identity, and open roles remain future phases. Preserve the distinction among raw Script, artifact, interface, and deployment identity. |
