...
The 0.30 branch implements steps 1 and 2 for artifact-known participants as
`cellscript-protocol-bundle-v1` and `cellscript-protocol-closed-role-v1`.
Concrete transaction materialization, shared-byte node execution, live
dependency validation, signing, submission, and bounded confirmation are also
implemented. Exact artifact receipts and fixed-width values now bind the
admitted package/entry/role, interface, typed semantics, ELF, target, ABI,
verified bundle, complete CKB Script, code CellDep, and chain identity.
Open/runtime selection and source choreography remain premature until the
generic checked construction and interface-parameterized Script-handle phases
are stable.
...
| [#9 Artifact-only multi-Script ProtocolBundle](https://github.com/CellScript-Labs/CellScript/issues/9) | **[ALIGNED] [OVERLAP] [IMPLEMENTED FORMAT] [PREREQUISITE]** | The 0.30 branch implements artifact admission, canonical composition, concrete transaction serialization, shared-byte node execution, live inputs/dependencies, external signing, tx-pool/submission, and bounded confirmation. A future `.celltx` file must remain a frontend over `cellscript-protocol-bundle-v1` or its accepted successor. Automatic builder-selected live Cells and independently attributed per-group cycles remain outside the present evidence. |
| [#10 Typed cross-Script transaction roles](https://github.com/CellScript-Labs/CellScript/issues/10) | **[ALIGNED] [IMPLEMENTED CLOSED ROLES] [PREREQUISITE] [SCOPE]** | `cellscript-protocol-closed-role-v1` reuses ProtocolBundle physical claims and binds exact Molecule schema, interface, ELF, and deployment identities for known participants. Open/runtime-selected roles remain a later phase built on #11 and must not be smuggled into ordinary entry parameters or imply runtime linkage. |
| [#11 Interface-bound runtime Script handles](https://github.com/CellScript-Labs/CellScript/issues/11) | **[ALIGNED] [IMPLEMENTED EXACT ARTIFACT RUNTIME IDENTITY] [DEPLOYMENT-LINE EXACT RUNTIME] [TYPE-HASH PROFILE] [PREREQUISITE]** | `cellscript-exact-script-handle-receipt-v1` and `CSHDLv1-fixed-202` bind exact independently checked ProtocolBundle artifacts and deployments while remaining ordinary non-Cell values. `ExactScriptHandle` is now a fixed source/ABI type whose checked CKB helpers bind the full handle hash to a selected Lock/Type Script hash or verifier CellDep data hash. `cellscript-deployment-line-receipt-v1` and `CSLINv1-fixed-386` define Type-hash exact-version history, six-dimensional compatibility, active/yanked, and admission commitments; the separate `ckb-type-hash` profile permits only Type-hash generated-artifact deployment. Standard TYPE_ID admission/code Cell evidence, exact admission transitions, mandatory ProtocolBundle binding, node-backed pre-signing liveness, fixed `DeploymentLineHandle`, and role-specific CKB-VM checks are implemented. Generic `ScriptHandle<I>` / `VerifierHandle<I>`, compatible-open identity, and open roles remain future phases. Preserve the distinction among raw Script, artifact, interface, and deployment identity. |
...
| [#17 Chain-identity-safe dependency selection](https://github.com/CellScript-Labs/CellScript/issues/17) | **[IMPLEMENTED PREREQUISITE]** | The 0.30 branch selects transitive environments by exact chain ID and genesis hash, records the complete decision in canonical lock node IDs, and rejects stale frozen mappings. Environment labels never substitute for chain identity. |
