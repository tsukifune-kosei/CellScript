...
| [#11 Interface-bound runtime Script handles](https://github.com/CellScript-Labs/CellScript/issues/11) | **[ALIGNED] [IMPLEMENTED EXACT ARTIFACT RUNTIME IDENTITY] [PREREQUISITE]** | `cellscript-exact-script-handle-receipt-v1` and `CSHDLv1-fixed-202` bind exact independently checked ProtocolBundle artifacts and deployments while remaining ordinary non-Cell values. `ExactScriptHandle` is now a fixed source/ABI type whose checked CKB helpers bind the full handle hash to a selected Lock/Type Script hash or verifier CellDep data hash. Generic `ScriptHandle<I>` / `VerifierHandle<I>`, upgrade-line identity, compatible-open identity, and open roles remain future phases. Preserve the distinction among raw Script, artifact, interface, and deployment identity. |
